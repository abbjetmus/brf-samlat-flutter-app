...
# Android: android/app/build.gradle.kts reads the version directly from pubspec.yaml,
# so there is NO separate Android file to edit — the pubspec bump above is enough.
# (local.properties' flutter.versionCode/Name is regenerated by `flutter build` and
#  is intentionally ignored by the Gradle config.)
echo "✓ Android reads the version from pubspec.yaml (android/app/build.gradle.kts) — versionCode $NEW_BUILD_NUMBER, versionName $NEW_VERSION_NAME"
